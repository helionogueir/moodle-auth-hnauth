...
if ($ADMIN->fulltree) {
    // Credentials
        ...
        $settings->add(new admin_setting_configtext(
                'auth_hnauth/password',
                (new lang_string('settings:credentials:password', 'auth_hnauth')),
                (new lang_string('settings:credentials:password:description', 'auth_hnauth')),
                '#@Sdf2dhdSFndgjd$#97434',
                PARAM_TEXT,
                25
        ));
}